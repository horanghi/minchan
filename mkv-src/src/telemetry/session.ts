 ...
 * - v5 → v6: 항목은 그대로인데 재는 대상이 달라졌다. 두 가지다.
 *   (1) 판이 다섯이 되어 `clears` 가 한 세션에서 여러 번 오른다 — v5 의 1 은
 *   "스테이지 1 클리어" 였지만 v6 의 3 은 "세 판을 지났다" 는 뜻이다.
 *   (2) 지상 적이 구덩이에 스스로 빠지지 않게 됐다. v5 의 스테이지 1 은
 *   좀비 절반이 알아서 사라진 판이었으므로 재시도율을 그대로 비교할 수 없다.
 */
