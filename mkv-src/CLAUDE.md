...
**여기가 소스의 집이다.** 민찬 게임랜드 저장소 안에서 관리한다.
고친 뒤에는 상위 폴더에서 `./build-mkv.sh` 를 돌려야 사이트에 반영된다 —
안 돌리면 아무 표시 없이 예전 빌드가 서비스된다. `./check-mkv-fresh.sh` 로 확인한다.
공개 주소는 **https://changame.vercel.app/mkv/** 다.
...
- **스테이지를 늘리거나 고칠 때는 `src/data/stages/stages.test.ts` 가 먼저다.**
  배치 규칙(구덩이 3타일, 머리 위 4타일, 착지 지점 적 없음, 상자·체크포인트 자리)을
  다섯 판 전부에 대해 검사한다. `clearable.test.ts` 는 봇으로 실제로 걸어 본다 —
  규칙을 다 지키고도 길이 막히는 경우가 있다.
