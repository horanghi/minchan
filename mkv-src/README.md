...
**https://mkv-five.vercel.app** — 스테이지 5판 (로그인 불필요)

보스를 잡으면 결과 화면에서 다음 판으로 이어진다. 진행도는 저장하지 않는다 —
이어 하기는 한 세션 안에서만이다.

| # | 스테이지 | 새로 나오는 것 |
|---|---|---|
| 1 | 서리 낀 묘지 | 조작 · 갑옷 · 붕괴 타일 · 그림 |
| 2 | 불타는 마령촌 | **불** — 닿으면 즉사하는 타일 |
| 3 | 얼어붙은 종루 | 원웨이 계단 · 붕괴 다리 |
| 4 | 속삭이는 늪 | 독물. 새 규칙 없이 밀도만 올린다 |
| 5 | 눈먼 갱도 | 천장 — 머리 위가 좁아진다 |

보스는 다섯 판 모두 캐른이고, 배경도 S1 한 벌을 함께 쓴다.
스테이지별 보스와 아트는 아직 없다 → [`docs/04`](docs/04-stages.md) 구현 현황
...
src/data/       밸런스 JSON · 스테이지 (stages.ts 의 배열이 곧 진행 순서)
```
